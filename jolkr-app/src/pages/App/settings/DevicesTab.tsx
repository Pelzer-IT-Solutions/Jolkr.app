  ...
  return (
    <>
      ...
      <div className="rounded-xl bg-surface border border-divider overflow-hidden">
        {loading && (
          <div className="text-text-muted text-sm text-center px-5 py-4">Loading devices...</div>
        )}
        {!loading && error && (
          <div className="text-error/70 text-sm text-center px-5 py-4">Failed to load devices</div>
        )}
        {!loading && !error && devices.length === 0 && (
          <div className="text-text-muted text-sm text-center px-5 py-4">No devices registered</div>
        )}
        {!loading && !error && devices.length > 0 && (
            ...
            {devices.map((device, idx) => (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-xs text-text-muted capitalize">{device.device_type}{device.has_push_token ? ' — push enabled' : ''}</div>
                </div>
                <button
