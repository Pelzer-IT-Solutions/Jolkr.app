...
export interface ThreadPanelProps {
  threadId: string;
  channelId: string;
  onThreadLoaded?: (thread: Thread) => void;
}

export default function ThreadPanel({ threadId, channelId, onThreadLoaded }: ThreadPanelProps) {
  ...
  useEffect(() => {
    ...
    api.getThread(threadId).then((t) => {
      if (!stale) {
        setThread(t);
        onThreadLoaded?.(t);
      }
    }).catch(() => { if (!stale) setThreadError(true); });
  ...
  }, [threadId, fetchThreadMessages, clearThreadMessages, onThreadLoaded]);
