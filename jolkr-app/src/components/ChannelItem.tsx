  ...
  return (
    <div className="group flex items-center min-w-0">
      <Link
        ...
        className={`
          flex-1 min-w-0 w-full rounded-md px-3 py-2 gap-2 text-left flex items-center text-sm cursor-pointer transition-colors no-underline
          ${isActive
      ...
      >
        {isVoice ? (
          <Mic className={`size-4.5 shrink-0 ${isActive ? 'text-text-primary' : 'text-text-tertiary'}`} />
        ) : (
          <span className={`text-lg font-medium shrink-0 ${isActive ? 'text-text-primary' : 'text-text-tertiary'}`}>#</span>
        )}
        <span className="truncate flex-1">{channel.name}</span>
        {isMuted && (
          <BellOff className="size-3.5 text-text-tertiary shrink-0" />
        )}
        {!isMuted && unreadCount > 0 && !isActive && (
          <Badge count={unreadCount} />
        )}
        {isInVoice && (
          <span className="size-2 rounded-full bg-online shrink-0" title="In voice" />
        )}
      </Link>
      {canManage && (
        <button
          onClick={() => onEdit(channel)}
          className="opacity-0 group-hover:opacity-100 focus:opacity-100 group-focus-within:opacity-100 p-1 text-text-tertiary hover:text-text-primary shrink-0"
          title="Edit Channel"
